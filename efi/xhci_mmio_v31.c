...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st) {
    ...
    UINT32 cmd=0,status=0,iman=0,maxslots=0,scratchpads=0,ps=0,slot_type_value=0,reads=0,writes=0;
    UINT32 event_type=0,completion=0,event_dw0=0,event_dw1=0,event_dw2=0,event_dw3=0,slot_id=0;UINT64 event_ptr=0;
    ...
    UINT32 *cr=NULL;UINT64 *dcbaa=NULL,*spa=NULL,*erst=NULL;UINT64 erst_rd=0,erdp_rd=0;
    ...
    s=slot_type(p,hcc,&slot_type_value,&ext_reads);reads+=ext_reads;if(EFI_ERROR(s)){fail(u"CAPS",u"SLOT TYPE",s);goto out;}Print(u"CAPS: DBOFF=%08x RTSOFF=%08x SLOT-TYPE=%u\r\n",db,rt,slot_type_value);
    ...
    s=reset_xhci(p,op,&cmd,&status,&reads,&writes);if(EFI_ERROR(s)){fail(u"RESET",u"RESET/CNR",s);goto out;}if(!(status&STS_HCH)){s=EFI_DEVICE_ERROR;fail(u"RESET",u"VERIFY HALTED",s);goto out;}
    ...
    dcbaa=(UINT64*)dcbaa_d.host;spa=(UINT64*)spa_d.host;cr=(UINT32*)cr_d.host;erst=(UINT64*)erst_d.host;dcbaa[0]=scratchpads?spa_d.dev:0;for(i=0;i<scratchpads;i++)spa[i]=sb[i].dev;
    cr[0]=0;cr[1]=0;cr[2]=0;cr[3]=TRB_CYCLE|(TRB_ENABLE_SLOT<<TRB_TYPE_SHIFT)|((slot_type_value&31U)<<16);cr[(CMD_TRBS-1)*4+0]=(UINT32)cr_d.dev;cr[(CMD_TRBS-1)*4+1]=(UINT32)(cr_d.dev>>32);cr[(CMD_TRBS-1)*4+2]=0;cr[(CMD_TRBS-1)*4+3]=TRB_CYCLE|TRB_LINK_TOGGLE|(TRB_LINK<<TRB_TYPE_SHIFT);
    ...
    s=mr32(p,ir,&iman);reads++;if(EFI_ERROR(s)){fail(u"RUN",u"IMAN",s);goto out;}s=mw32(p,ir,(iman&~IMAN_IE)|(iman&IMAN_IP));writes++;if(EFI_ERROR(s)){fail(u"RUN",u"DISABLE IRQ",s);goto out;}s=mr32(p,op,&cmd);reads++;if(EFI_ERROR(s)){fail(u"RUN",u"USBCMD",s);goto out;}halted=FALSE;s=mw32(p,op,(cmd&~(CMD_INTE|CMD_HSEE))|CMD_RUN);writes++;if(EFI_ERROR(s)){fail(u"RUN",u"START",s);goto out;}s=wait_hch(p,op,FALSE,1000,&status,&reads);if(EFI_ERROR(s)){fail(u"RUN",u"HCH CLEAR",s);goto out;}
    s=mw32(p,db,0);writes++;if(EFI_ERROR(s)){fail(u"COMMAND",u"DOORBELL",s);goto out;}submitted=TRUE;Print(u"ENABLE SLOT: DOORBELL=0 COMMAND-TRB=%016lx SLOT-TYPE=%u\r\n",cr_d.dev,slot_type_value);
    for(i=0;i<10000;i++){volatile UINT32 *e=(volatile UINT32*)ev_d.host;event_dw0=e[0];event_dw1=e[1];event_dw2=e[2];event_dw3=e[3];if((event_dw3&TRB_CYCLE)&&((event_dw3&TRB_TYPE_MASK)>>TRB_TYPE_SHIFT)==TRB_COMMAND_COMPLETION)break;uefi_call_wrapper(BS->Stall,1,1000);}
