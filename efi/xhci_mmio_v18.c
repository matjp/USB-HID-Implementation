...
static void finish(UINT32 reads, UINT32 changes) {
    Print(u"\r\nV18 COMPLETE / %u READ-ONLY PORTSC MMIO READS / NO DMA / NO WRITES\r\n", reads);
    Print(u"PORT VALUE CHANGES BETWEEN PASSES: %u\r\n", changes);
    if (changes == 0)
        Print(u"BASELINE: STABLE\r\n");
    else
        Print(u"BASELINE: CHANGED -- INVESTIGATE\r\n");
    Print(u"EXIT %u SEC...\r\n", EXIT_SEC);
...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st) {
    ...
    UINT32 caplen, max_ports, op_base, off, portsc, reads=0, changes=0;
    UINT32 baseline[256];
    UINT32 pass, port;
    UINT64 bar;

    InitializeLib(image, st);
    for (i=0; i<256; ++i) baseline[i]=0;

    Print(u"TOSHIBA xHCI V18 / 5-PASS BASELINE\r\n");
    Print(u"READ-ONLY / NO DMA / NO WRITES\r\n");

    s = LibLocateHandle(ByProtocol, &PciGuid, NULL, &n, &hs);
    if (EFI_ERROR(s)) {
        Print(u"PCI ENUM FAIL %r\r\n", s);
        finish(reads, changes);
        return s;
    }
    ...
    for (pass=1; pass<=PASSES; ++pass) {
        ...
        for (port=1; port<=max_ports; ++port) {
            ...
            if(EFI_ERROR(s)) {
                Print(u"PORT%02u MMIO[%03x]=FAIL %r\r\n",port,off,s);
                if(hs)FreePool(hs);
                finish(reads, changes);
                return s;
            }
            ...
            if (pass == 1) {
                baseline[port]=portsc;
                Print(u"PORT%02u MMIO[%03x]=%08x CCS=%u PED=%u PP=%u PLS=%u\r\n",
                      port,off,portsc,portsc&1,(portsc>>1)&1,
                      (portsc>>9)&1,(portsc>>5)&0xf);
            } else if (portsc != baseline[port]) {
                ++changes;
                ++pass_changes;
                Print(u"CHANGE PASS%u PORT%02u %08x -> %08x\r\n",
                      pass,port,baseline[port],portsc);
                baseline[port]=portsc;
            }
        }

        if (pass > 1)
            Print(u"PASS %u CHANGES=%u\r\n",pass,pass_changes);
