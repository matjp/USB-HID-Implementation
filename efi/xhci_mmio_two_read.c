...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st) {
    ...
    UINT32 cap0 = 0, hcs1 = 0;
    UINT64 bar;

    InitializeLib(image, st);
    Print(u"TOSHIBA xHCI V7 / TWO-MMIO READS\r\n");
    ...
    /* MMIO read #1: xHCI capability header at BAR + 0. */
    s = mmio32(p, 0, &cap0);
    if (EFI_ERROR(s)) {
    ...
    /* MMIO read #2: HCSPARAMS1 at capability-space offset 0x04. */
    s = mmio32(p, 4, &hcs1);
    if (EFI_ERROR(s)) {
