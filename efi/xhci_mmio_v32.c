...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st)
{
    ...
    /* Gate-3/Gate-4 infrastructure, carried forward from V29/V30. */
    ...
    /* Primary event ring segment table: one 16-TRB segment. */
    erst[0] = ev_d.dev;
    ((UINT32 *)erst_d.host)[2] = EVENT_TRBS;
    ((UINT32 *)erst_d.host)[3] = 0;

    /* No CPU interrupt delivery during this gate. */
    s = mw32(p, ir + 0x00, 0);
    ...
    s = mw32(p, op + 0x18, maxslots & 0xffU);
    ...
    /* Verify controller pointers before starting it. */
    {
        UINT64 v;
        s = mr64(p, op + 0x30, &v);
        if (EFI_ERROR(s) || v != dcbaa_d.dev) {
            s = EFI_DEVICE_ERROR;
            remember_fail(u"INIT", u"DCBAAP READBACK", s);
            goto out;
        }
        s = mr64(p, op + 0x18, &v);
    ...
    /* Start xHCI exactly as proven by V30. */
    s = mr32(p, op, &cmd);
    ...
    /* Port must still be connected after cumulative start. */
    ...
    /* Gate-5 carried forward: Enable Slot using the Supported Protocol
     * capability covering the discovered root port. */
    {
...

    {
        ...
        /* Full Input Context is zeroed by DMA allocation. A0 and A1 are the
         * only Add Context flags used by Address Device. */
        ic[0] = 0x3U;
        ic[1] = 0;
        ic[2] = 0;
        ic[3] = 0;

        /* Slot Context: route string 0, discovered port speed, one endpoint,
         * root-hub port, interrupter target 0, all hub/TT fields zero. */
        sc[0] = (speed << SPEED_SHIFT) | (1U << CTX_ENTRIES_SHIFT);
        sc[1] = d.port << ROOT_PORT_SHIFT;
        sc[2] = 0;
        sc[3] = 0;

        /* EP0 is DCI 1, represented by the first endpoint context after slot. */
        ...
        /* Output Device Context is independently zeroed and its DCBAA pointer
         * is already installed before Address Device. */
        dc[0] = 0;
        dc[1] = 0;
        dc[2] = 0;
        dc[3] = 0;
    ...
    /* Teardown rule: after Address Device, the xHC owns the output context
     * until the slot is disabled. Disable the slot before freeing its memory. */
    {
        UINT32 *r = (UINT32 *)cr_d.host;
        UINT64 cmd_addr = cr_d.dev + cmd_index * 16U;
        r[cmd_index * 4 + 0] = 0;
    ...
    Print(u"V32 ADDRESS DEVICE: PASS SLOT=%u / DISABLE SLOT: PASS\r\n", slot);
    Print(u"V32 COMPLETE / CUMULATIVE DISCOVERY + PORT RESET + ADDRESS DEVICE\r\n");
    Print(u"NO DESCRIPTORS / NO CONFIGURE ENDPOINT / NO HID REPORTS\r\n");

out:
    if (hs) uefi_call_wrapper(BS->FreePool, 1, hs);

    /* Any path that may have left the xHC running must never free DMA. */
