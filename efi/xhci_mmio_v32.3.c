...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st)
{
    ...
    UINT64 *dcbaa = NULL, *erst = NULL;
    UINT32 *erst_entry = NULL;

    InitializeLib(image, st);
    Print(u"TOSHIBA xHCI V32.3 / PORT STATE DIAGNOSTIC\r\n");
    ...
    /* Primary interrupter: ERSTSZ=1, ERDP=event ring base, ERSTBA=table. */
    s = mw32(p, 0x20 + 0x08, 1U);
    if (EFI_ERROR(s)) goto out;
    s = mw64(p, 0x20 + 0x18, ev_d.dev | 8ULL);
    if (EFI_ERROR(s)) goto out;
    s = mw64(p, 0x20 + 0x10, erst_d.dev);
    if (EFI_ERROR(s)) goto out;
