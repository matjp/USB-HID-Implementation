...
static void done(UINT32 reads) {
    Print(u"\r\nREAD-ONLY / %u PORTSC MMIO READS / NO DMA / NO WRITES\r\n", reads);
    Print(u"EXIT %u SEC...\r\n", DELAY_SEC);
...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st) {
    ...
    UINT32 max_ports;
    UINT32 caplen, op_base, port_off;
    UINT64 bar;
    UINT32 reads = 0;

    InitializeLib(image, st);
    Print(u"TOSHIBA xHCI V15 / PORT14-16 REPEAT\r\n");
    ...
    if (max_ports < LAST_PORT) {
        Print(u"NOT ENOUGH PORTS FOR TEST\r\n");
        if (hs)
            FreePool(hs);
        done(reads);
    ...
    for (UINT32 pass = 1; pass <= PASSES; ++pass) {
        ...
        for (UINT32 port = FIRST_PORT; port <= LAST_PORT; ++port) {
            UINT32 portsc = 0;

            port_off = op_base + 0x400 + ((port - 1) * 0x10);
