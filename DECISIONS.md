...
The portable mechanism selected for this EFI experiment is the UEFI Driver Model `DisconnectController()` operation on the selected xHCI controller handle, with recursive child disconnection enabled. The implementation must verify that the disconnect succeeds before proceeding. After that point the bridge must not use `EFI_USB_IO_PROTOCOL`, UEFI USB timers, or UEFI-created xHCI runtime resources. The bridge then establishes fresh xHCI controller/device state and its own DMA mappings.
